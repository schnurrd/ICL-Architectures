...
class MainConfig(base_config.BaseConfig):
    ...
    # Debugging
    debug_spike_enabled: bool = False
    debug_spike_threshold: float = 3.0
    debug_spike_max_saves: int = 50

    # Loss jump handling
    loss_jump_enabled: bool = False
    restart_on_loss_jump: bool = False
        ...
        for epoch in range(start_epoch, c.epochs + 1):
            ...
            try:
                epoch_result = train_or_evaluate_epoch(
                    ...
                    last_epoch_result=prev_epoch_result,
                )
            ...
            loss_jump = (
                c.loss_jump_enabled
                and epoch > c.warmup_epochs
                and prev_epoch_result is not None
                and epoch_result.loss > 1.5 * prev_epoch_result.loss
            )
            if loss_jump:
                if c.restart_on_loss_jump and c.train_state_dict_save_path is not None:
                    print(
                        f"Loss jump detected (epoch {epoch}). Reloading last checkpoint."
                    )
                    load_checkpoint(
                        model,
                        optimizer,
                    ...
                    continue
                raise ValueError(
                    f"Aborting training due to loss jump: {epoch_result.loss}"
                )
            ...
            prev_epoch_result = epoch_result
            ...
            try:
                with autocast(
                    ...
                    if c.debug_spike_enabled:
                        spike_save_count = log_loss_spike(
                            loss=loss,
                            nan_share=nan_share,
                            threshold=c.debug_spike_threshold,
                            single_eval_pos=single_eval_pos,
                            n_targets_per_input=c.n_targets_per_input,
                            categorical_inds=categorical_inds,
                            batch=batch,
                            ...
                            training=training,
                            rank=rank,
                            train_state_dict_save_path=c.train_state_dict_save_path,
                            spike_save_count=spike_save_count,
                            spike_save_max=c.debug_spike_max_saves,
                            epoch=epoch,
                ...
                if batch_index % c.aggregate_k_gradients == c.aggregate_k_gradients - 1:
                    ...
                    if math.isfinite(grad_norm):
                        ...
                        if c.debug_spike_enabled and is_spike and not in_warmup:
